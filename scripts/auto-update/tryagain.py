...
# ── 单日下载 ──────────────────────────────────────────────────
def retry_day(date_str: str, stats: Stats, update_file: str = UPDATE_FILE) -> None:
...
def run_retry_round(
    ...
    if no_tui:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(retry_day, d, stats, update_file): d for d in dates}
...
if __name__ == "__main__":
    ...
    main(no_tui=args.no_tui, workers=args.workers, data_dir=args.data_dir, update_file=args.update_file)
